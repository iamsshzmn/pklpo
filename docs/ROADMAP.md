...
## Текущее состояние (обновлено 2026-07-22 по факту кода и коммитов)

> Roadmap 3.0 (текст ниже) писался 2026-06-03. С тех пор Фаза A ушла заметно
> дальше, чем зафиксировано в гейтах, и появились две крупные ветки работы,
> которых в исходном roadmap нет вообще — **identity layer** (continuous
> series / PIT-adjusted facade) и **TON→GRAM rebrand migration** (symbol
> succession). См. раздел «Незапланированная работа (обнаружена 2026-07-22)»
> ниже. Строки таблицы, статус которых изменился с 2026-05-20, помечены `[upd]`.

| Слой | Состояние | Комментарий |
|------|-----------|-------------|
| Ingest / candles | ✅ работает | `ON CONFLICT` во всех репозиториях, идемпотентность ingestion есть |
| Repair / backfill | 🟡 код есть `[upd]` | Recovery controller + dry-run gate + decisions store добавлены (PR #5); реальный gap-прогон (A7) всё ещё не подтверждён явно |
| Identity layer (series/continuous OHLCV) `[new]` | 🟡 код есть | `core.series_registry/segments/continuous_ohlcv_p/v_ohlcv_facade` — не было в исходном roadmap, см. `.claude/rules/analytics.md` |
| TON→GRAM rebrand `[new]` | ✅ смерджено (PR #7) | symbol succession + gap classification поверх identity layer |
| Features pipeline | 🟡 работает, раздут | 158 файлов / ~28.7k строк — оценочно ×3-5 от нужного объёма; prune ещё не проводился |
| ML labeling / validation | 🟡 тесты есть `[upd]` | `tests/ml/`: `test_triple_barrier.py`, `test_validation.py`, `test_numerical_equivalence.py`, `test_lookahead.py`, `test_sample_weights.py` — уже не "тестов 0", но coverage % и mlfinlab-сверка не подтверждены |
| ML metalabeling | ⚪ код впереди потребности | Нет decision-log и накопленных сделок для обучения — заморожено |
| Signals | 🟡 скелет | Без изменений — `signals/` есть; `signal_decisions`/`manual_trades` всё ещё не существуют |
| signal_decisions / manual_trades | 🔴 не существует | Критичный missing piece для research-цели — Фаза B не начата |
| Telegram notifier | 🟡 частично `[upd]` | `TelegramAlertObserver` в `src/features/infrastructure/alerts.py` шлёт ops-алерты (WARNING+); это не B2 (сигналы → Telegram), а побочный продукт observability-трека |
| Backtest | 🟡 частично | Custom `calc_pnl`; vectorbt/quantstats всё ещё не в `pyproject.toml` — миграция (B6) не начата |
...
| Backup / DR | ⚪ снято с плана `[upd 2026-07-22]` | Решение владельца: некуда копировать (нет второго диска/облака). Риск принят; данные восстановимы через re-sync с OKX. A0 закрыт как won't-do, A1 (аудит размера БД) понижен до P1 |
| `make check` / CI quality gate | 🟡 зелёный впервые `[upd]` | `gh run list --branch main --workflow CI` (проверено 2026-07-22): **каждый push на `main` падал с самого первого коммита (2026-03-02) до и включительно PR #6 (2026-06-27)** — это первый в истории проекта зелёный прогон, а не стабилизировавшаяся практика. `make lookahead` подключён в `make check` (A3 закрыт по конфигурации). Один зелёный прогон не доказывает A6 ("починить до зелёного" как устойчивое состояние) — нужно минимум ещё 1-2 зелёных мерджа подряд, прежде чем считать гейт закрытым |

**Главный диагноз (частично устарел):** проект продвинулся в Фазе A (тесты
написаны, look-ahead gate подключён, CI наконец прошёл впервые). Бэкапы (A0)
сняты с плана 2026-07-22 решением владельца — некуда копировать, риск принят.
Оставшийся открытый риск Фазы A — **CI зелен только один раз**, а не как
устойчивая практика. Фаза B (baselines, signal_decisions, research workflow,
vectorbt-миграция) не начата вообще. Приоритет: (1) удержать CI зелёным ещё
на 1-2 мерджа подряд прежде чем считать A6/A8 закрытыми, (2) **актуализировать
сам roadmap** регулярно — иначе следующий agent-run снова будет работать по
устаревшему диагнозу.
...
## Незапланированная работа (обнаружена 2026-07-22)

Две ветки работы вошли в `main` за последний ~1.5 месяца и не описаны нигде в
Roadmap 3.0. Roadmap нужно либо расширить под них, либо явно решить, что это
«вне scope v1, отдельный трек» — пока это не зафиксировано, они не имеют
DoD/гейта и не проверяются на regression относительно целей v1.

| Ветка | Что сделано | Статус | Открытый вопрос |
|-------|-------------|--------|------------------|
| **Identity layer** (continuous series) | `core.series_registry`, `core.series_members`, `core.series_alias`, `core.series_segments`, `core.series_gap_ranges`, `core.continuous_ohlcv_p`, `core.v_ohlcv_facade` — PIT-adjusted continuous bars поверх symbol-keyed таблиц (см. `.claude/rules/analytics.md`, обновлено 2026-07-06) | 🟡 код есть, `ExecutionResolver` для symbol↔series_id | Нужен ли это для v1 (research/discovery на 1D/4H/1W) или это подготовка под мультипериодные ребренды/делистинги — решение владельца |
| **TON→GRAM rebrand migration** (PR #7, смерджено 2026-07-16) | Symbol succession + gap classification для случая ребрендинга инструмента, recovery gate | ✅ смерджено, CI зелёный | Разовый кейс или паттерн, который повторится (ожидать ещё ребренды/делистинги)? Если паттерн — стоит завести как явный ID в Фазе A/B, а не оставлять implicit |

**Рекомендация:** на ближайшем еженедельном review явно завести ID (например,
`A10` — identity layer hardening) под identity layer, если она остаётся в
scope v1 — иначе она молча увеличивает раздутость (см. Prune Track), которую
Roadmap и так называет главной tech-debt проблемой.
...
Цель: фундамент, которому можно доверять. Тесты на код, который утверждает
«бэктест честный», доказанная идемпотентность. (Бэкапы БД сняты с плана —
см. A0 ниже.)

| ID | Задача | Приоритет |
|----|--------|-----------|
| ~~A0~~ | ~~Бэкапы БД (3-2-1: второй диск + облако).~~ **Снято с плана 2026-07-22** — нет второй точки хранения (второго диска/облака), копировать некуда. Данные воспроизводимы через re-sync с OKX (см. `candles`), поэтому DB-бэкап не единственный путь восстановления — риск принят владельцем. | — |
| A1 | Аудит размера БД: `pg_total_relation_size` по таблицам. Понять, из чего 70 GB, почистить bloat. Прогноз роста под рост глубины/инструментов. | P1 |
...
- [x] ~~Бэкап восстановлен из облака в чистое окружение хотя бы раз.~~ Снято — некуда копировать (нет второго диска/облака); данные восстановимы через re-sync с OKX.
...
dup-rate = 0; double-run тесты всех DAG зелёные. (Бэкап/restore снят из
критерия 2026-07-22 — некуда копировать; восстановление — через re-sync с OKX.)
...
---
*Roadmap 3.0 — результат discovery-аудита. Последнее обновление: 2026-07-22
(сверка с фактическим кодом/коммитами: CI зелёный на `main`, ml-тесты
написаны, добавлен раздел про identity layer и TON→GRAM как незапланированную
работу; backup/DR подтверждённо отсутствует). Предыдущее обновление:
2026-06-03 (observability, CI/CD, Redis, structured logging).*
