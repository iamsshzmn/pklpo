...
from src.candles.infrastructure.runtime_adapters import build_runtime_market_adapter
...
class SwapCandlesSync:
    ...
    def _build_market_adapter(self) -> MarketDataAdapterPort:
        return build_runtime_market_adapter(self.config)
    ...
    async def sync_swap_bar(
        ...
        latest_stored_ts: int | None = None,
    ) -> tuple[int, str | None]:
        """
        ...
        try:
            ...
            if latest_stored_ts is not None:
                candles = [
                    candle for candle in candles if int(candle["ts"]) > latest_stored_ts
                ]
                if not candles:
                    logger.debug(
                        "%s %s: no candles newer than %s",
                        symbol,
                        timeframe,
                        latest_stored_ts,
                    )
    ...
    async def sync_swap_symbol(
        ...
        async def sync_one_tf(tf: str) -> tuple[str, int]:
            ...
            latest_stored_ts = await self.repository.fetch_latest_timestamp_ms(
                symbol=symbol,
                timeframe=tf,
            )
            logger.debug("Start sync for %s %s", symbol, tf)
            while True:
                count, last_ts = await self.sync_swap_bar(
                    symbol,
                    tf,
                    before_local,
                ...
                if latest_stored_ts is not None and last_ts is not None and int(last_ts) <= latest_stored_ts:
                    logger.debug(
                            "%s %s: reached stored boundary at %s",
                            symbol,
                            tf,
                            latest_stored_ts,
                        )
