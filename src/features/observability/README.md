...
Observability Layer обеспечивает мониторинг, логирование и сбор метрик для модуля features. Позволяет отслеживать производительность, качество данных и диагностировать проблемы.
...
├── indicators_logging.py  # DEPRECATED - используйте logging.py
├── logging.py             # Конфигурация логгеров (основной модуль)
...
# Новый код:
from src.features.observability.logging import LogAggregator, LogCategory
...
```bash
pytest src/features/tests/test_logging_config.py -v
pytest src/features/tests/test_metrics.py -v
```
