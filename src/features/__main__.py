...
# Иначе используем старый cli.py интерфейс
cli_file_path = os.path.join(os.path.dirname(__file__), "cli.py")
spec = importlib.util.spec_from_file_location("src.features.cli", cli_file_path)
if spec is None or spec.loader is None:
    raise ImportError(f"Failed to load cli.py from {cli_file_path}")

cli_module = importlib.util.module_from_spec(spec)
cli_module.__package__ = "src.features"
cli_module.__name__ = "src.features.cli"
