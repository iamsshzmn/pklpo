...
Refactored (Task 8): Uses FeatureCalculator Protocol via FeatureCalculationService
for better testability and DIP compliance.
"""
...
def process_chunks(
    ...
    **kwargs,
    ...
    Args:
        ...
        calculator: FeatureCalculator instance (DI); if None, uses default service
    ...
    with set_log_context(symbol=symbol, timeframe=timeframe) as run_id:
        logger.info(
            f"Processing chunks with overlap={overlap_size}, max_lookback={max_lookback}, run_id={run_id}"
        )
            ...
            for chunk_df in reader:
                ...
                if overlap_data is not None:
                    ...
                    if (
                        ...
                        combined_df = pd.concat(
                            [overlap_data, chunk_df], ignore_index=True
                        )
                ...
                try:
                    with memory_monitor(f"calculate_chunk_{chunk_count}") as chunk_mem:
                        features_df = calculator.calculate(
                            combined_df,
                            specs=(
                            ...
                            volatility_normalize=config.volatility_normalize,
                            **kwargs,
                        )
...
def compute_and_dump_parquet(
    ...
    **kwargs,
    ...
    try:
        ...
        # Calculate indicators via injected calculator
        logger.info(f"Calculating indicators for {symbol} {timeframe}")
        result_df = calculator.calculate(
            df_ohlcv,
            specs=list(available_indicators) if available_indicators else None,
