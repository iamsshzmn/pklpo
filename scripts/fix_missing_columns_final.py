#!/usr/bin/env python3
"""
Финальный скрипт для исправления отсутствующих колонок ics_26 и rma_20
"""
...
async def check_and_fix_missing_columns():
    ...
    try:
        ...
        missing_columns = []
        required_columns = ["ics_26", "rma_20", "t3_20"]  # Все колонки из логов ошибок
        ...
        for col in required_columns:
            if col not in current_columns:
                ...
                logger.info(f"✅ Колонка {col} уже существует: {current_columns[col]}")
            ...
            for col in missing_columns:
                try:
                    ...
                    if "duplicate column" in str(e).lower():
                        logger.info(f"ℹ️ Колонка {col} уже существует (дублирование)")
                    else:
        ...
        if all(col in final_columns for col in required_columns):
            logger.info("🎉 ПРОБЛЕМА РЕШЕНА! Все необходимые колонки добавлены в БД")
            return True
        missing_final = [col for col in required_columns if col not in final_columns]
        logger.error(f"❌ Проблема не решена - отсутствуют колонки: {missing_final}")
        return False
