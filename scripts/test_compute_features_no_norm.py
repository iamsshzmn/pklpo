#!/usr/bin/env python3
"""
Тест для отладки compute_features БЕЗ нормализации
"""
...
def test_compute_features_no_norm():
    """Тестируем compute_features БЕЗ volatility normalization"""
    print("=== ТЕСТ COMPUTE_FEATURES БЕЗ NORMALIZATION ===")
    ...
    # Тестируем с небольшим набором индикаторов БЕЗ нормализации
    test_specs = ["ema_8", "sma_20", "rsi_14"]

    print(f"\nТестируем compute_features с specs: {test_specs}")
    print("volatility_normalize=False")

    try:
        result_df = compute_features(
            df_ohlcv=df,
            specs=test_specs,
            volatility_normalize=False,  # ОТКЛЮЧАЕМ НОРМАЛИЗАЦИЮ!
        )
