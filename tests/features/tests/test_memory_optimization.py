...
from src.features.domain.strategy import get_max_lookback_for_strategies, max_lookback
...
async def test_database_operations():
    ...
    from src.features.application.save import _prepare_batch_data

    batch_data = _prepare_batch_data(df, "TEST", "1H")
    print(f"  Prepared {len(batch_data)} batch records")

    # Test batch data validation
    from src.features.application.save import _validate_dataframe
