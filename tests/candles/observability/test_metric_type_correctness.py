...
_PROMETHEUS_PY = _PROJECT_ROOT / "src" / "candles" / "observability" / "prometheus.py"
_DASHBOARD_JSON = (
    _PROJECT_ROOT
    / "ops"
    / "monitoring"
...
def test_swap_sync_per_run_gauges_have_no_total_suffix() -> None:
    ...
    fn_match = re.search(r"def push_swap_sync_metrics.*?(?=\ndef |\Z)", src, re.DOTALL)
...
def test_no_total_gauge_in_dashboard_under_rate() -> None:
    ...
    assert not bad, f"Dashboard still uses rate() on _total Gauge series: {bad}"
